...
<div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper">
	...
	<div class="container">
		<div class="row">
			...
			<h3 class="card-title"> Update Product SEO</h3>
		</div>
		<!--begin::Form-->
		<form method="post" action="{{route('admin.seo.update.product',$subseo->id)}}">
							@csrf
			<div class="row">
				<div class="col-md-12">
					<div class="box">
							<div class="box-content">
								...
								<div class="form-group-select">
									<lable class="col-sm-3 col-lg-3 control-label">Chose SubCategory<span style="color: #a94442;">*</span></lable>
									<div class="col-sm-6 col-lg-4 controls">
										<select class="form-control select select1" aria-label="Default select example" name="subcategory_id" id="subcategory_id">
								
											@foreach($subcategorys as $subcategory)
												@if($subcategory->status == '1')
												<option data-value="{{$subcategory->category_id}}" value="{{$subcategory->id}}" @if($subcategory->id == $subseo->subcategory_id) selected @endif>{{ $subcategory->subcategory}}</option>
												@endif
											@endforeach
								...
								<div class="form-group-select">
									...
									<div class="col-sm-6 col-lg-4 controls">
										<select class="form-control select1" aria-label="Default select example" name="product_id" id="product_id">
								
											@foreach($products as $product)
												@if($product->status == '1')
												<option data-value="{{$product->subcategory_id}}" value="{{$product->id}}" @if($product->id == $subseo->product_id) selected @endif>{{ $product->product_name}}</option>
												@endif
											@endforeach
...
// to determine which select's option and value would be changed
// after you select an option in another select.
var selectors = ['subcategory_id', 'product_id']
