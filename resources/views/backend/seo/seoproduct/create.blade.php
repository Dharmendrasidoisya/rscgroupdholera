...
<div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper">
	...
	<div class="container">
		<div class="row">
			...
			<h3 class="card-title"> Add Product SEO</h3>
		</div>
		<!--begin::Form-->
		<form method="post" action="{{route('admin.seo.storeproduct')}}">
							@csrf
			<div class="row">
				<div class="col-md-12">
					<div class="box">
										...
										@if($categorys->isNotEmpty())
											@foreach ($categorys as $category)
												@if($category->status == '1')
													<option data-value="0" value="{{$category->id}}" >{{$category->category_name}}</option>
												@else
												@endif
											@endforeach
										@else
									...
									<div class="col-sm-6 col-lg-4 controls">
										...
										<option value="" selected >--Select Product--</option>
											...
											@foreach($products as $product)
												@if($product->status == '1')
												<option data-value="{{$product->subcategory_id}}" value="{{$product->id}}" >{{ $product->product_name}}</option>
												@endif
											@endforeach
