...
<div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper">
	...
	<div class="container">
		<div class="row">
			...
			<h3 class="card-title"> View SubCategory SEO</h3>
		</div>
		<!--begin::Form-->
					...
					<div class="box">
							<div class="box-content">
									...
									<div class="col-sm-6 col-lg-4 controls">
										<select class="form-control select" aria-label="Default select example" name="subcategory_id" id="subcategory_id">
											...
											@foreach($products as $product)
													@if($product->status == '1')
													<option data-value="{{$product->category_id}}" value="{{$product->id}}" @if($product->id == $subseo->subcategory_id) selected @endif>{{ $product->product_name}}</option>
													@endif
											@endforeach
								...
								<div class="form-group-select">
									...
									<div class="col-sm-6 col-lg-4 controls">
										<input name="meta_title" type="text"class="form-control" data-rule-required="true" data-rule-minlength="3" placeholder="Meta Title" value="{{$subseo->subcategory}}"/>
										<span class="text-danger">
											@error("meta_title")
											{{$message}}
											@enderror
